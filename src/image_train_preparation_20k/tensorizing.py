...
def download_data(
    source_bucket_name, csv_blob_name, local_image_dir, local_csv_path, sample_size
):
    ...
    # Sample `sample_size` (default 20k) images
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-sb",
        "--source_bucket_name",
        default="215-multiclass-car-bucket",
    ...
    parser.add_argument(
        "-db",
        "--dest_bucket_name",
        default="tensor-bucket-20k",
        help="Destination GCS bucket name",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        default="/mnt/c/Users/Harper/Documents/GitHub/github_no_ship",
        help="Local directory to save temporary files",
    )
    parser.add_argument(
        "-ss", "--sample_size", default=20000, help="Number of images to tensorize"
    )
